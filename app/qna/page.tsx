    ...
    <div className="container py-8">
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">약에 관한 궁금한 점을 질문하고 전문가의 답변을 받아보세요.</p>
        </div>

        <div className="flex flex-col gap-4 sm:flex-row">
          <div className="flex w-full items-center space-x-2">
            <Input type="text" placeholder="질문 검색" value={searchQuery} onChange={handleSearch} />
        ...
        <Tabs defaultValue="expert" className="w-full">
          <TabsList>
            ...
            <TabsTrigger value="community">커뮤니티 Q&A</TabsTrigger>
          </TabsList>
          <TabsContent value="expert" className="mt-4">
            <div className="grid gap-4">
              {filteredQuestions
                .filter((question) => question.type === "expert")
                .map((question) => (
          ...
          <TabsContent value="community" className="mt-4">
            <div className="grid gap-4">
              {filteredQuestions
                .filter((question) => question.type === "community")
                .map((question) => (
                  <Link href={`/qna/${question.id}`} key={question.id}>
                    <Card className="h-full overflow-hidden transition-all hover:shadow-md">
                      <CardContent className="p-6">
                        <div className="flex items-start gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <Badge variant="outline">커뮤니티 Q&A</Badge>
                              <div className="flex flex-wrap gap-1">
                                {question.tags.map((tag) => (
                                  <Badge key={tag} variant="outline" className="text-xs">
                                    {tag}
