...
import { Search, Plus, Filter, Pencil, Trash, Save, X } from "lucide-react"
...
export default function InventoryPage() {
  ...
  const handleAddMedicine = () => {
    ...
    const newItem = {
      id: newId,
      ...newMedicine,
      status: determineStatus(newMedicine.stock, newMedicine.minStock, newMedicine.expiryDate),
    }
    ...
    const updatedInventory = inventory.map((item) => {
      if (item.id === id) {
        return {
          ...item,
          ...editFormData,
          status: determineStatus(editFormData.stock, editFormData.minStock, editFormData.expiryDate),
        }
      }
      return item
    })
  ...
  const determineStatus = (stock: number, minStock: number, expiryDate: string) => {
    ...
    <div className="container py-8">
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">약국의 재고를 관리하고 부족한 약품을 확인하세요.</p>
        </div>

        <div className="flex flex-col gap-4 sm:flex-row">
          <div className="flex w-full items-center space-x-2">
            <Input type="text" placeholder="약품명, 제조사, 유형 검색" value={searchQuery} onChange={handleSearch} />
          ...
          <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="medicine-minStock">최소 재고 수량</Label>
                  <Input
                    id="medicine-minStock"
                    type="number"
                    placeholder="최소 재고 수량을 입력하세요"
                    value={newMedicine.minStock.toString()}
                    onChange={handleInputChange}
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="medicine-expiryDate">유통기한</Label>
                  <Input
                    ...
                    <div className="divide-y">
                      {filteredInventory.length > 0 ? (
                        filteredInventory.map((item) => (
                          <div key={item.id} className="grid grid-cols-7 gap-4 p-4 items-center">
                            {editingItem === item.id ? (
                              // Edit mode
                              <>
                                <div>
                              ...
                              <>
                                ...
                                <div className="text-sm">{item.expiryDate}</div>
                                <div>{getStatusBadge(item.status)}</div>
                                <div className="flex gap-2">
                                  <Button size="sm" variant="outline" onClick={() => handleEditMedicine(item.id)}>
                                    <Pencil className="h-4 w-4" />
                                  </Button>
                                  <Button size="sm" variant="outline" onClick={() => handleDeleteClick(item.id)}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span>재고 부족 약품</span>
                    <span className="font-bold text-yellow-500">{lowStockItems.length}개</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>만료 임박 약품</span>
                    <span className="font-bold text-orange-500">{expiringItems.length}개</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {lowStockItems.length > 0 ? (
                      ...
                      <div key={item.id} className="flex items-center justify-between p-2 border rounded-md">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            현재: {item.stock} / 최소: {item.minStock}
                          </p>
                        </div>
                        <Button size="sm" variant="outline" onClick={() => handleEditMedicine(item.id)}>
                          수정
                    ...
                    <p className="text-center text-muted-foreground">재고 부족 약품이 없습니다.</p>
                  )}
                </div>
              </CardContent>
            </Card>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              정말로 이 약품을 재고 목록에서 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
