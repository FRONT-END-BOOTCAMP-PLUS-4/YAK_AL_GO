...
/**
 * 효능군중복 API 응답 구조 인터페이스
 */
export interface EfficacyDuplicationApiResponseInterface {
  header: {
  ...
  body: {
    ...
    items: EfficacyDuplicationDataInterface[]; // 실제 데이터 배열
...
/**
 * 효능군중복 정보 조회 서비스 클래스
 * 공공데이터포털 DUR 효능군중복 API와 연동하여 중복투여 주의 정보를 관리
 */
class EfficacyDuplicationService {
  // 공공데이터포털 DUR 효능군중복 API 엔드포인트
   ...
   * @returns 효능군중복 정보 배열
   */
  async getEfficacyDuplicationByItemSeq(
    itemSeq: string
  ): Promise<EfficacyDuplicationDataInterface[]> {
    try {
      console.log(`효능군중복 정보 조회 시작: ${itemSeq}`);

      const response = await this.fetchEfficacyDuplicationData(1, 100, itemSeq);

      if (!response.body.items || response.body.items.length === 0) {
        console.log(`효능군중복 정보 없음: ${itemSeq}`);
        return [];
      }

      console.log(`효능군중복 정보 조회 완료: ${response.body.items.length}건`);
      return response.body.items;
    } catch (error) {
      console.error(`효능군중복 정보 조회 오류 (${itemSeq}):`, error);
      return [];
    }
  }
   ...
   * @returns 효능군중복 정보 배열
   */
  async getEfficacyDuplicationByItemName(
    itemName: string
  ): Promise<EfficacyDuplicationDataInterface[]> {
    try {
      console.log(`효능군중복 정보 조회 시작 (약품명): ${itemName}`);

      const response = await this.fetchEfficacyDuplicationData(1, 100, undefined, itemName);

      if (!response.body.items || response.body.items.length === 0) {
        console.log(`효능군중복 정보 없음 (약품명): ${itemName}`);
        return [];
      }

      console.log(`효능군중복 정보 조회 완료 (약품명): ${response.body.items.length}건`);
      return response.body.items;
    } catch (error) {
      console.error(`효능군중복 정보 조회 오류 (약품명: ${itemName}):`, error);
      return [];
    }
  }
   ...
   * @returns API 응답 데이터
   */
  private async fetchEfficacyDuplicationData(
    pageNo: number,
    numOfRows: number,
    itemSeq?: string,
    itemName?: string
  ): Promise<EfficacyDuplicationApiResponseInterface> {
    try {
      // API 요청 파라미터 설정
      ...
      console.log(`효능군중복 API 호출: 페이지 ${pageNo}, 요청 건수 ${numOfRows}`);
      ...
      const response = await axios.get(this.API_BASE_URL, {
        ...
        headers: {
          'User-Agent': 'DUR-EfficacyDuplication-Service/1.0',
      ...
      const data = response.data as EfficacyDuplicationApiResponseInterface;
      ...
      console.log(`효능군중복 API 응답 성공: ${data.body.items?.length || 0}건 수신`);
      return data;
    } catch (error) {
      console.error('효능군중복 API 호출 상세 에러:', error);
      const errorMessage = error instanceof Error ? error.message : '알 수 없는 오류';
      throw new Error(`효능군중복 API 호출 실패: ${errorMessage}`);
    }
  }
...
export { EfficacyDuplicationService };
